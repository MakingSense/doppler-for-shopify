...
export function dopplerListCreated(
  createdDopplerListId,
  createdDopplerListName
) {
  return {
    type: types.DOPPLER_LIST_CREATED,
    createdDopplerList: {
      value: parseInt(createdDopplerListId),
      label: createdDopplerListName,
    },
  };
}
...
export function changeCurrentSelectedList(selectedListId = null) {
  return {
    ...
    selectedListId:
      selectedListId != null ? parseInt(selectedListId) : selectedListId,
  };
}
  ...
  return (dispatch, getState) => {
    ...
    return appService
      ...
      .then(response => {
        ...
        dispatch(
          dopplerListsRetrieved(
            response.items.map(list => {
              return { value: list.listId, label: list.name };
            })
          )
        );
      })
  ...
  return (dispatch, getState) => {
    ...
    return appService
      ...
      .then(response => {
        ...
        } else dispatch(duplicatedListName());
      })
      .catch(errorPromise => {
        dispatch(creatingDopplerList(false));
        dispatch(requestListCreation(false));
...
export function setDopplerList(
  selectedListId,
  dopplerListName,
  setupCompleted
) {
