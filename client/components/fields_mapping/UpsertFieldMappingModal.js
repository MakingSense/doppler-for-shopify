...
class UpsertFieldMappingModal extends Component {
  ...
  componentDidMount() {
    ...
    const shopifyFieldValue = shopifyFieldsOptions.length > 0 ? shopifyFieldsOptions[0].value : null;
  ...
  getShopifyFieldHelpText(shopifyFieldValue) {
    ...
    const shopifyField = this.props.shopifyFields.find(sf => sf.path === shopifyFieldValue);
  ...
  getDopplerFieldHelpText(dopplerFieldValue) {
    ...
    const dopplerField = this.props.dopplerFields.find(df => df.name === dopplerFieldValue);
    return `Type: ${dopplerField.type}.${dopplerField.sample ? ` Sample: ${dopplerField.sample}` : ''}`;
  }
  ...
  getShopifyOptions() {
      return this.props
        ...
        .map(f => { return {value: f.path, label: f.name} })
  }

  getDopplerFieldsOptions(shopifyFieldValue) {
    return this.props
        .dopplerFields
        .filter(d => !this.props.fieldsMapping.some(m => m.doppler === d.name) 
            && this.props.shopifyFields.find(f => f.path === shopifyFieldValue).type === d.type
        ).map(d => { return {value: d.name, label: d.name} });
  }

  handleShopifyFieldChange(shopifyFieldValue) {
    const dopplerFieldsOptions = this.getDopplerFieldsOptions(shopifyFieldValue);
    const dopplerFieldValue = dopplerFieldsOptions.length > 0 ? dopplerFieldsOptions[0].value : null;
    const shopifyFieldHelpText = this.getShopifyFieldHelpText(shopifyFieldValue);
    const dopplerFieldHelpText = this.getDopplerFieldHelpText(dopplerFieldValue);
    this.setState({
        dopplerFieldsOptions,
        shopifyFieldValue,
        dopplerFieldValue,
...
export default connect(mapStatesToProps, mapDispatchToProps)(UpsertFieldMappingModal);