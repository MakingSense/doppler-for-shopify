...
const app = proxyquire('./', {
    ...
    'connect-redis': (session) => { return undefined; }
});
...
describe('Server integration tests', function() {
    ...
    beforeEach(function () {
        ...
        this.sandbox.stub(mocks.wrappedRedisClient, 'quit')
            .callsFake((cb) => { cb(); });
        this.sandbox.stub(mocks.wrappedRedisClient, 'hmset')
            .callsFake((key, obj, cb) => { cb(); });
        this.sandbox.stub(mocks.wrappedRedisClient, 'del')
            .callsFake((key, cb) => { cb(); });
    });
    ...
    describe('GET /shopify/auth/callback', function(){
        it('Should generate the session cookie with the access token', async function() {
            ...
            fetchStub.withArgs(`https://${shopDomain}/admin/oauth/access_token`, {
                method: 'POST',
                headers: {
            ...
            gotStub.returns(Promise.resolve({
                headers: { 'x-shopify-shop-api-call-limit': '1/9999' },
                body: {}
            }));

            const message = `code=1234567890&shop=${shopDomain}&timestamp=${(new Date()).getTime()}`;
    ...
    describe('GET /', function() {
        ...
        it('Should redirect to "shopify/auth?shop={shop}" when session does not exist and shop is passed as parameter', async function() {
            await request(app)
                ...
                .expect(function(res) {
                    expect(res.statusCode).to.be.eql(302);
                    expect(res.headers.location).to.be.eql(`/shopify/auth?shop=${shopDomain}`);
                });
        });
        ...
        it ('Should render the home page when there is an existing session', async function() {
            this.sandbox.stub(mocks.wrappedRedisClient, 'hgetall')
              .callsFake((key, cb) => { cb (null, {accessToken: accessToken, dopplerAccountName: dopplerAccountName}); });
        ...
        it ('Should redirect to /shopify/auth?shop={shop} when access token has changed', async function() {
            this.sandbox.stub(mocks.wrappedRedisClient, 'hgetall')
              .callsFake((key, cb) => { cb (null, {accessToken: '111111111111', dopplerAccountName: dopplerAccountName}); });
    ...
    describe('POST /connect-to-doppler', function() {
        ...
        it ('Should return 401 status code when invalid Doppler credentials', async function() {
            fetchStub.withArgs(`https://restapi.fromdoppler.com/accounts/${querystring.escape(dopplerAccountName)}`, 
    ...
    describe('GET /doppler-lists', function() {
        it ('Should return the Doppler lists correctly', async function() {
            this.sandbox.stub(mocks.wrappedRedisClient, 'hgetall')
              .callsFake((key, cb) => { cb (null, {accessToken, dopplerAccountName, dopplerApiKey}); });
    
            fetchStub.withArgs(`https://restapi.fromdoppler.com/accounts/${querystring.escape(dopplerAccountName)}/lists?page=1&per_page=200&state=active`, 
              { headers: { Authorization: `token ${dopplerApiKey}` } 
            }).returns(Promise.resolve({
              status: 200,
              json: async function() {
            ...
            await request(app)
                ...
                .set('cookie', cookie)
                .expect(function(res) {
                    expect('{"items":[{"listId":1459381,"name":"shopify"},{"listId":1222381,"name":"marketing"},{"listId":1170501,"name":"development"}],"itemsCount":3}')
                        .to.be.eql(res.text);
                    expect(200).to.be.eql(res.statusCode);
                });
        });
        ...
        it ('Should return 500 status code when there is not a Doppler API key', async function() {
            this.sandbox.stub(mocks.wrappedRedisClient, 'hgetall')
              .callsFake((key, cb) => { cb (null, { accessToken }); });
    ...
    describe('POST /create-doppler-list', function() {
        it ('Should create the Doppler list correctly', async function() {
            ...
            fetchStub.withArgs(`https://restapi.fromdoppler.com/accounts/${querystring.escape(dopplerAccountName)}/lists`,
                { body: '{"name":"Fresh List"}', method: "POST", headers: { Authorization: `token ${dopplerApiKey}` } })
            .returns(Promise.resolve({
                status: 201,
                json: async function() {
    ...
    describe('GET /fields', function() {
        it ('Should return the Doppler fields correctly', async function() {
            ...
            fetchStub.withArgs(`https://restapi.fromdoppler.com/accounts/${querystring.escape(dopplerAccountName)}/fields`,
                { headers: { Authorization: `token ${dopplerApiKey}` } })
            .returns(Promise.resolve({
                status: 201,
                json: async function() {
    ...
    describe('POST /synchronize-customers', function() {
        it ('Should return 201 status code on success', async function() {
            this.sandbox.stub(mocks.wrappedRedisClient, 'hgetall')
            .callsFake((key, cb) => { cb (null, {shop: shopDomain, accessToken, dopplerAccountName, dopplerApiKey, fieldsMapping, dopplerListId}) });
            
            const expectedRequestBody = JSON.stringify(dopplerApiPayloads.IMPORT_SUBSCRIBERS_PAYLOAD);

            fetchStub.withArgs(`https://restapi.fromdoppler.com/accounts/${querystring.escape(dopplerAccountName)}/lists/${dopplerListId}/subscribers/import`,
                { body: expectedRequestBody, method: "POST", headers: { Authorization: `token ${dopplerApiKey}` } })
            .returns(Promise.resolve({
                status: 202,
                json: async function() {
    ...
    describe('POST /hooks/customers/created', function() {
        it ('Should return 200 status code on success', async function() {
            ...
            const hmac = crypto
                .createHmac('sha256', process.env.SHOPIFY_APP_SECRET)
                .update(Buffer.from('{"id":623558295613,"email":"jonsnow@example.com","first_name":"Jon","last_name":"Snow","default_address":{"company":"Winterfell"}}'))
                .digest('base64');
            ...
            await request(app)
                ...
                .set('X-Shopify-Topic', 'app/uninstalled')
                .set('X-Shopify-Shop-Domain', shopDomain)
                .send('{"id":623558295613,"email":"jonsnow@example.com","first_name":"Jon","last_name":"Snow","default_address":{"company":"Winterfell"}}')
                .expect(function(res) {
                    expect(200).to.be.eql(res.statusCode);
            });
        });
